...
function workspaceHasLocalChanges(workspace: ActiveWorkspace): boolean {
  return diffDocuments(workspace.lastSynced, doc).length > 0;
}

function updateWorkspaceChip(): void {
  const chip = app.querySelector<HTMLButtonElement>('#workspaceChip');
  const label = app.querySelector<HTMLElement>('#workspaceLabel');
  if (!chip || !label) return;
  chip.classList.toggle('on', Boolean(activeWorkspace));
  chip.classList.toggle('conflict', Boolean(activeWorkspace?.error));
  label.textContent = activeWorkspace
    ? `agent · r${activeWorkspace.revision}`
    : 'agent · local';
  chip.title = activeWorkspace
...
async function syncWorkspace(force = false): Promise<boolean> {
  ...
  try {
    ...
    workspace.revision = result.revision;
    workspace.lastSynced = target ?? structuredClone(doc);
...
function adoptWorkspaceSnapshot(
  ...
  workspace.error = null;
  loadDoc(snapshot.document, false);
  writeWorkspaceLink();
  updateWorkspaceChip();
}
...
async function handOffCurrentDocument(): Promise<void> {
  ...
  try {
    const snapshot = await createWorkspace(structuredClone(doc));
    activeWorkspace = {
      id: snapshot.id,
    ...
    alert(error instanceof Error ? error.message : 'Workspace handoff failed.');
  } finally {
...
async function restoreWorkspace(): Promise<void> {
  ...
  try {
    ...
    activeWorkspace = {
      ...
      pendingTarget: saved.pending ? structuredClone(doc) : null,
      syncing: false,
      paused: false,
      status: 'reconnecting…',
      error: null,
    ...
    } else if (workspaceFingerprint(doc) === saved.syncedFingerprint) {
...
function closeWorkspacePanel(): void {
  ...
  workspacePanel = null;
  app
    .querySelector<HTMLButtonElement>('#workspaceChip')
...
function renderWorkspacePanel(): void {
  ...
  body.querySelector('#wsLease')?.addEventListener('click', () => {
    void (async () => {
      ...
      await grantWorkspaceLease(workspace.id, editor.page.id, 600);
      await refreshWorkspaceState(false);
    })().catch((error) => {
      workspace.error = error instanceof Error ? error.message : String(error);
      renderWorkspacePanel();
...
function enableWorkspaceUi(): void {
  ...
  const chip = app.querySelector<HTMLButtonElement>('#workspaceChip');
  const divider = app.querySelector<HTMLElement>('#workspaceDiv');
...
window.addEventListener('beforeunload', () => {
  ...
  if (workspace.pending) writeWorkspaceLink(workspace.pending);
    ...
    const operations = diffDocuments(workspace.lastSynced, doc);
    writeWorkspaceLink(
      operations.length
        ? {
            baseRevision: workspace.revision,
