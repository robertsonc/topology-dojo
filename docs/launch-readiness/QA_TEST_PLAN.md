...
| F8-14 | Session lifecycle (remote): create doc, idle past DO eviction, reconnect                                                                            | Documented behavior verified: in-memory store is gone; `get_topology` fails cleanly with guidance (this is a _known constraint_ — test the failure mode, not persistence) |
