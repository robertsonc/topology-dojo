...
| AUTH-11 | API keys (proposal 0005): mint/list/revoke are cookie-only and owner-scoped; the plaintext appears once; storage holds only a hash; `/keys` redirects when signed out; the flag-off 503 contract holds | Automated (unit + Miniflare); Required mint → use → revoke journey on staging                                   |
| AUTH-12 | A valid `tdk_` bearer on `/mcp` resolves to its owner's identity with exactly its scopes; wrong/expired/revoked/unknown keys and non-`/mcp` routes get 401; failures are budgeted per IP               | Automated resolver + scope gate; Required against the real Worker wrapper (smoke `mcp-apikey-unauth` + staging) |
...
| MCP-10 | `element.upsert` in proposals and leased applies converges by source identity (add then patch, ids stable, in-batch idempotent, create-required fields enforced) without changing the stored vocabulary; `get_topology sources:true` and `get_workspace_elements sourcedOnly:true` return only sourced elements; `edit_topology` reports `created` for upserts | Automated (unit + Miniflare); remote-transport run Required on staging |
