...
- Live-data connector tools (`list_appliances`, `list_flows`, `build_flow_topology`, …) against a **real** EdgeConnect Orchestrator — tested against `TOPOLOGY_PROVIDER=mock` only, unless a fabric sandbox is provisioned before T+20. Real-fabric integration is a launch-blocker *only if* live-data ships enabled in prod.
...
| P0 | **MCP correctness on the Worker** — Durable Object session isolation, tool parity with stdio, renderer parity (bundled vs `createRequire`) | The agent story is the product's differentiator | F8, N3 |
...
| F8-13 | Session isolation (remote): two concurrent MCP clients each `create_topology`; each calls `list_topologies` | Each sees **only its own** documents (one DO per session) |
| F8-14 | Session lifecycle (remote): create doc, idle past DO eviction, reconnect | Documented behavior verified: in-memory store is gone; `get_topology` fails cleanly with guidance (this is a *known constraint* — test the failure mode, not persistence) |
...
3. **Automated E2E (Playwright), nightly against staging:** F8-02 golden agent loop over HTTP `/mcp` (with a pre-provisioned OAuth token), F5-01→F5-02 share-link loop, F6-01/02 login loop, F2-02 duplicate-page invariant, F2-04 autosave reload. ~15 scenarios max — keep it fast and unflaky.
...
| ID | Test | Expected |
|---|---|---|
| N3-01 | 25 concurrent MCP sessions each running the F8-02 loop against staging `/mcp` (scripted MCP client) | All succeed; per-session isolation holds (spot-check `list_topologies`); p95 tool latency < 2 s (render < 5 s) |
