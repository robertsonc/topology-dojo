...
- **Status (2026-08-17): Closed.** Both halves of the fix landed in one publish path (`worker/share.ts`): (1) every publish (browser `/api/share` POST and MCP `share_topology`) records the snapshot in the owner's KV index (`shares:<uid>`, uid = stable numeric GitHub id), and revocation — the browser Share dialog's per-link **revoke**, `DELETE /api/share/:id`, and the MCP `revoke_share` tool — deletes `doc:<id>` and the index record, with ownership enforced through the index so one user can never delete another's snapshot; (2) `serveSnapshot` now sends `Cache-Control: public, max-age=300` (no `immutable`), so a revoked link stops resolving within a bounded ~5-minute cache window. Verified by a Miniflare suite through the real default handler (`src/testing/share-api.test.ts`: publish → list → public serve without `immutable` → foreign-owner revoke rejected with 404 → owner revoke → public 404). Pre-existing snapshots published before the index existed still expire on their original 30-day TTL but cannot be listed/revoked (documented in the user guide).
