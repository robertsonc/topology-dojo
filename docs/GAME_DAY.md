...
| Live-data fabric tools      | `LIVE_DATA_ENABLED` = `"false"` (opt-in; unset also off) | `"false"` or remove the entry — fail-closed flag        | Seven live-data MCP tools unregistered even if `ORCH_*` secrets are present; authoring / workspace / profile tools unaffected. Remaining blast radius when on: every authenticated MCP session on the deployment can query the connected fabric unless `LIVE_DATA_GITHUB_IDS` restricts the grant. | MCP `tools/list` omits `list_appliances` / `list_flows` / `build_flow_topology` (and the other four live-data names) | `"true"` (plus env-specific `ORCH_*` secrets; optional `LIVE_DATA_GITHUB_IDS`) | MCP `tools/list` includes the seven live-data tools for an allowed owner                      |
...
(`ORCH_BASE_URL`/`ORCH_API_KEY`) is gated by `LIVE_DATA_ENABLED` (opt-in,
currently `"false"` in both environments) independently of secret
presence, so provisioning secrets cannot silently open fabric read access
(issue #228). Revisit this row if packet E5 provisions a non-production
Orchestrator and an operator flips the flag.
